...
serve(async (req) => {
  ...
  try {
    ...
    for (const model of ordered) {
      ...
      // Try every candidate object in the reply, and accept the array under
      // whatever name the model chose — demanding exactly "lines"/"rows" was
      // throwing away usable answers.
      let parsed: any = null;
      for (const cand of jsonCandidates(content)) {
        if (kind === "invoice") {
          ...
          const rows = findArray(cand, ROW_KEYS);
          if (rows) {
            const columns = Array.isArray(cand.columns) ? cand.columns
              : (rows.length && typeof rows[0] === "object" && !Array.isArray(rows[0])
                  ? Object.keys(rows[0]) : []);
            // rows may arrive as objects; flatten them to columns order
            const normRows = rows.map((r: any) => Array.isArray(r)
              ? r : columns.map((c: string) => (r?.[c] ?? "")));
            parsed = { title: cand.title ?? null, columns, rows: normRows };
            break;
          }
        }
      }
