                ...
                <thead className="bg-secondary">
                  <tr>
                    {([['quoteNumber', Hash, 'N°'], ['customerName', User, 'Client'], ['createdAt', Calendar, 'Date'], ['totalAmount', DollarSign, 'Montant'], ['status', Filter, 'Statut']] as const).map(([field, Icon, label]) => (
                      <th key={field} className="px-3 py-2 text-left text-[11px] font-medium text-muted-foreground uppercase tracking-wider cursor-pointer hover:bg-accent" onClick={() => handleSort(field)}>
