 ...
 * so bulk recipients don't all receive byte-identical messages (a known spam
 * signal). Handles nesting by resolving innermost groups first. Double braces
 * ({{var}}) are untouched — variables are substituted separately.
 */
export function spin(text: string): string {
  let out = text || '';
  // innermost {a|b} group: no braces inside, at least one pipe
  const group = /\{([^{}|]*(?:\|[^{}|]*)+)\}/;
  for (let guard = 0; guard < 50; guard++) {
    const m = out.match(group);
    if (!m) break;
...
/** Fill {{name}}, {{phone}}, {{field}} from a contact; unknown vars → ''. Spins variants, appends CTA link. */
...
export class WaCampaignsService {
  ...
  /** Contacts matched by a campaign's segment, minus opted-out and known-invalid numbers. */
  static async resolveAudience(filter: SegmentFilter): Promise<WaContact[]> {
    const [contacts, outs, eng] = await Promise.all([
      WaContactsService.list(),
      WaContactsService.listOptOuts(),
      filter.behavior ? WaContactsService.engagement() : Promise.resolve(undefined),
    ]);
    const optedOut = new Set(outs.map(o => o.phone));
    return contacts.filter(c =>
      !optedOut.has(c.phone) && c.wa_status !== 'invalid' && matchSegment(c, filter, eng));
  }
